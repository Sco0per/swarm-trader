...
2. `swing-trader live-ticket <snapshot.json>` runs that snapshot through the **exact same, unchanged** deterministic risk engine (`src/swing/risk.py`) that governs paper trading — the same 0.75% baseline, reduce-only regime/drawdown rules, position/day/week limits, open-risk limits, and durable halts — and prints either a rejection reason or an approved order ticket (exact quantity, entry, stop, target, planned dollar risk). It never calls a broker's order-placement method; `HumanSuppliedBrokerProvider` (`src/swing/brokers/human_supervised.py`) structurally cannot place, cancel, or modify an order — those methods always raise, by design, not as a pending stub.
...
- `RobinhoodMCPProvider` (`src/swing/brokers/robinhood_mcp.py`) remains an intentionally non-executing skeleton. It is not used by any command today. Order-placement methods on any Robinhood provider should stay permanently disabled regardless, per the reasoning above.

## One narrow, deliberate exception: automated public market-data reads

2026-08-13: the "read side automated" question above was partially resolved, scoped tightly to **public market data only, never account state or orders**. `volume-relay-agent` (see `docs/CRON_AGENTS.md`'s "Real-volume relay routine") is a scheduled routine that calls the Robinhood MCP connector's `get_equity_historicals` tool unattended, to work around Alpaca's IEX-only feed understating real trading volume by 10-50x. This was verified against Robinhood's actual live tool schema from an authenticated session, not guessed — confirmed via two real unattended test routine runs, including the exact tool name (`mcp__Robinhood_agent__get_equity_historicals`) and that no permission prompt blocks an unattended call.

This does not weaken anything above: the tool it calls returns public OHLCV bars for a requested symbol, exactly like Alpaca's own bars endpoint — it cannot read the account's positions/cash/equity, place an order, or touch the dedicated Agentic account in any way. It is explicitly restricted (`docs/CRON_AGENTS.md`) to that one read-only tool, never `place_option_order`, `place_equity_order`, `review_*_order`, or any watchlist-mutation tool from the same connector. The decisive reasoning in "Why this framework never automates Robinhood order placement" above — no sandbox, per-session human-supervised permission model — still fully applies to anything that could move money or touch the account, and remains untouched.
