...
class BacktestService:
    ...
    def __init__(
        ...
        if float(portfolio.get("margin_requirement", 0.0)) != 0.0 or float(portfolio.get("margin_used", 0.0)) != 0.0:
            raise ValueError("Margin is unsupported in the long-only framework")
    ...
    def execute_trade(self, ticker: str, action: str, quantity: float, current_price: float) -> int:
        ...
        if action in {"short", "cover"}:
            raise ValueError("Short selling is unsupported in the long-only framework")
        ...
        if action == "buy":
            ...
            available_cash = max(0.0, self.portfolio["cash"] - self.portfolio["margin_used"])
