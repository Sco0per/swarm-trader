...
async def run(request_data: HedgeFundRequest, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        portfolio = create_portfolio(request_data.initial_cash, 0.0, request_data.tickers, request_data.portfolio_positions)
...
async def backtest(request_data: BacktestRequest, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        portfolio = create_portfolio(request_data.initial_capital, 0.0, request_data.tickers, request_data.portfolio_positions)
