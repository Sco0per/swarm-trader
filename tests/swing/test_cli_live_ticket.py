...
def test_live_ticket_blocked_outside_live_mode(monkeypatch, tmp_path, capsys, candidate, proposal):
    ...
    assert "EXECUTION_MODE=live" in capsys.readouterr().err
...
def test_record_live_fill_requires_and_consumes_deterministic_ticket_with_protection(monkeypatch, tmp_path, capsys, candidate, proposal):
    ...
    assert cli_module.main() == 2
    assert "No unused deterministic live-ticket approval" in capsys.readouterr().err
