...
def add_open_trade(database, settings, proposal, candidate):
    ...
    database.add_trade({
        ...
        "ticker": "XYZ", "setup_type": candidate.setup_type.value, "status": "OPEN",
...
def test_closed_trade_creates_postmortem_and_observation(database, settings, proposal, candidate):
    ...
    record = PostmortemEngine(database).close_and_review(
        trade_id, exit_price=96, high_during_trade=102, low_during_trade=95.5, reason_exit="technical stop",
    )
...
class _StubPostmortemBackend:
    ...
    def complete(self, *, role, model_name, payload, schema):
        ...
        self.calls += 1
        return schema(
            classification=self.classification, followed_strategy=True, entry_valid=True, regime_correct=True,
            sector_favorable=True, relative_strength_favorable=True, stop_sensible=True, target_realistic=True,
            execution_hurt=False, unexpected_event=False, rule_rationalization=False, normal_variance=False,
            repeats_known_lesson=False, mistake_type=None, reasoning="clean trend continuation win",
        )
...
def test_close_and_review_uses_llm_backend_for_valid_win(database, settings, proposal, candidate):
    ...
    record = PostmortemEngine(database).close_and_review(
        trade_id, exit_price=108, high_during_trade=109, low_during_trade=99, reason_exit="target hit",
        backend=backend, model_name="claude-opus-5",
    )
...
def test_close_and_review_backend_failure_falls_back_to_deterministic_default(database, settings, proposal, candidate):
    ...
    record = PostmortemEngine(database).close_and_review(
        trade_id, exit_price=96, high_during_trade=102, low_during_trade=95.5, reason_exit="technical stop",
        backend=_BrokenBackend(), model_name="claude-opus-5",
    )
    assert record.classification.value == "VALID_LOSS"
...
def test_autoresearch_requires_multiple_observations(database, settings, tmp_path):
    ...
    with pytest.raises(ValueError):
        engine.propose(
            observation="one loss", proposed_rule="ban ticker", supporting_lesson_ids=["one"],
            supporting_sample_size=1, setup_type=None, market_regime=None, candidate_strategy_version="SWING_V1.1_CANDIDATE",
        )
...
def test_validation_never_auto_promotes(database, settings, tmp_path):
    ...
    hypothesis_id = engine.propose(
        observation="breakouts weak in declining QQQ", proposed_rule="require rising QQQ 20MA",
        supporting_lesson_ids=["a", "b", "c", "d", "e"], supporting_sample_size=30,
        setup_type="BREAKOUT_RETEST", market_regime="NEUTRAL", candidate_strategy_version="SWING_V1.1_CANDIDATE",
    )
    baseline = FitnessMetrics(expectancy_r=0.1, profit_factor=1.2, sharpe=0.5, sortino=0.7, max_drawdown=0.06,
                              total_return=0.08, trade_count=50, turnover=2, regime_stability=0.5,
                              performance_concentration=0.4, strategy_complexity=3)
    candidate = baseline.model_copy(update={"expectancy_r": 0.25, "profit_factor": 1.6, "sharpe": 0.9,
                                            "sortino": 1.2, "max_drawdown": 0.05, "regime_stability": 0.7})
    result = engine.record_comparison(
        hypothesis_id, baseline=baseline, candidate=candidate, data_hash="data", config_hash="config",
        train_period="2018-2021", validation_period="2022-2023", out_of_sample_period="2024-2025",
        regime_results={"BULL": {"expectancy_r": 0.3}, "CHOPPY": {"expectancy_r": 0.1}},
        benchmark={"SPY": 0.1, "QQQ": 0.12}, walk_forward={"positive_out_of_sample": True},
    )
